...
def get_discriminator(latent_extractor_ckpt, discriminator_ckpt, condition, img_resolution=32, device='cuda', enable_grad=True, depth=2):
    classifier = load_classifier(latent_extractor_ckpt, img_resolution, device, eval=True, in_channels=4)
    discriminator = load_discriminator(discriminator_ckpt, device, condition, depth=depth, eval=True, text_cond=True)
    def evaluate(perturbed_inputs, timesteps=None, condition=None, sigmoid=True):
        with torch.enable_grad() if enable_grad else torch.no_grad():
            ...
            prediction = discriminator(adm_features, timesteps, sigmoid=sigmoid, condition=condition).view(-1)
        return prediction
    return evaluate
