...
class StableDiffusionRephrasePipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        prompt, prompt_rephrase = prompt[0], prompt[1]
        ...
        prompt_embeds_org = self._encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        # 3. Encode input prompt
        prompt_embeds_rephrase = self._encode_prompt(
            prompt_rephrase,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps[:-1]):
                ...
                noise_pred = self.unet(
                    torch.cat([latent_model_input, latents]),
                    t,
                    encoder_hidden_states=torch.cat([prompt_embeds_org, prompt_embeds_rephrase[batch_size:]], dim=0),
                    cross_attention_kwargs=cross_attention_kwargs,
                ).sample

                noise_pred = torch.cat([noise_pred[:batch_size], (noise_pred[batch_size:2*batch_size] + noise_pred[2*batch_size:]) / 2], dim=0)
