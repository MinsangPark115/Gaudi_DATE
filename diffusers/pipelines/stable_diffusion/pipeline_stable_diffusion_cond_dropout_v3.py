...
class StableDiffusionCondDropoutv3Pipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if timestep != -1:
                if lst_indices_to_replace is None:
                    lst_indices_to_replace = []
                    for i in range(len(text_input_ids)):
                        pad_start = (text_input_ids[i] == 49407).nonzero(as_tuple=True)[0]
                        pad_start = pad_start[0].item() if len(pad_start) > 0 else len(text_input_ids[0])

                        indices = torch.arange(1, pad_start)
                        k = int(torch.floor((pad_start - 1) * (timestep / 1000)**self.time_power).item())
                        if k != 0:
                            indices_to_replace = indices[torch.randperm(indices.size(0))[:k]]
                            text_input_ids[i, indices_to_replace] = 1
                        lst_indices_to_replace.append(indices_to_replace)
                else:
                    for i in range(len(text_input_ids)):
                        pad_start = (text_input_ids[i] == 49407).nonzero(as_tuple=True)[0]
                        pad_start = pad_start[0].item() if len(pad_start) > 0 else len(text_input_ids[0])

                        indices = lst_indices_to_replace[i]
                        prev_k = int(torch.floor((pad_start - 1) * (prev_timestep / 1000) ** self.time_power).item())
                        k = int(torch.floor((pad_start - 1) * (timestep / 1000)**self.time_power).item())
                        if prev_k - k >= 1:
                            indices_to_replace = indices[torch.randperm(indices.size(0))[:k]]
                        else:
                            indices_to_replace = lst_indices_to_replace[i]
                        text_input_ids[i, indices_to_replace] = 1
                        lst_indices_to_replace[i] = indices_to_replace
                print(timestep, k, text_input_ids)
        ...
        return prompt_embeds, lst_indices_to_replace
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps[:-1]):
                ...
                # 3. Encode input prompt
                prompt_embeds, lst_indices_to_replace = self._encode_prompt(
                    prompt,
                    device,
                    num_images_per_prompt,
                    do_classifier_free_guidance,
                    ...
                    negative_prompt_embeds=negative_prompt_embeds,
                    timestep=t,
                    lst_indices_to_replace=lst_indices_to_replace,
                    prev_timestep=timesteps[:-1][i-1],
                )
