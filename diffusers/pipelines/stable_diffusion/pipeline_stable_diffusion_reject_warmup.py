...
class StableDiffusionRejectionWarmupPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            if discriminator is not None:
                if warmup:
                    log_ratio_prev = discriminator(latents, torch.ones(latents.shape[0], device=latents.device) * timesteps[0] / 999, prompt_embeds[len(latents):], sigmoid=False).detach().cpu()
                    for j in range(len(log_ratio_prev)):
                        adaptive[0].append(log_ratio_prev[j].cpu())
                else:
                    while bool_zero.sum() != 0:
                        x_check = latents[:]
                        labels_ = prompt_embeds[len(latents):][:]
                        log_ratio_prev_check = log_ratio_prev[:]
                        log_ratio = classifier_lib.get_grad_log_ratio(discriminator, vpsde, latents, sigmas[0].cuda().repeat(len(latents)), 64, 0.01, 1, prompt_embeds[len(latents):], log_only=True).detach().cpu()
                        bool_neg_log_ratio = log_ratio < adaptive[lst_idx][:] + torch.log(torch.rand_like(log_ratio) + 1e-7)
                        bool_reject = torch.arange(len(labels_), device=labels_.device)[:][bool_neg_log_ratio]
                        bool_accept = torch.arange(len(labels_), device=labels_.device)[:][~bool_neg_log_ratio]
            ...
            for i, t in enumerate(timesteps[:-1]):
                ...
                latents_next = self.scheduler.step(noise_pred, lst_idx, latents, **extra_step_kwargs).prev_sample

                # Apply 2nd order correction (Heun).
                if second_order and i < len(timesteps) - 2:
                    t_next = timesteps[lst_idx+1]
                    ...
                    latents = self.scheduler.step(0.5 * noise_pred + 0.5 * noise_pred_prime, i, latents,
                ...
                if discriminator is not None:
                    if warmup:
                        log_ratio = discriminator(latents, torch.ones(latents.shape[0], device=latents.device) * timesteps[i+1] / 999, prompt_embeds[len(latents):], sigmoid=False).detach().cpu()
                        for j in range(len(log_ratio)):
                            adaptive[i+1].append(log_ratio[j].cpu())
                        for j in range(len(log_ratio)):
                            adaptive2[i+1].append(log_ratio[j].cpu() - log_ratio_prev[j].cpu())
        ...
        if warmup:
            return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept), image_list, adaptive, adaptive2
        else:
