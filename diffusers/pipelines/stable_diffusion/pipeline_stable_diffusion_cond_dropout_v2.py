...
class StableDiffusionCondDropoutv2Pipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
        ...
        if prompt_embeds is None:
            ...
            if timestep != -1:
                for i in range(len(text_input_ids)):
                    ...
                    k = int(torch.floor((pad_start - 1) * (timestep / 1000)**7).item())
                    if k != 0:
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps[:-1]):
                ...
                if t > 700:
                    lst_prompt_embeds = []
                    for np in range(n_prompt_sample):
                        prompt_embeds_temp = self._encode_prompt(
                            prompt,
                            device,
                            num_images_per_prompt,
                            do_classifier_free_guidance,
                        ...
                        lst_prompt_embeds.append(prompt_embeds_temp)
                    ...
                    for np in range(n_prompt_sample):
                        noise_pred = noise_pred + self.unet(
                            latent_model_input,
                            t,
                            encoder_hidden_states=lst_prompt_embeds[np],
                            cross_attention_kwargs=cross_attention_kwargs,
                        ).sample
                    noise_pred = noise_pred / n_prompt_sample
